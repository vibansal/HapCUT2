...
int sample_block(struct fragment* Flist, int fragments, struct SNPfrags* snpfrag, int start, int end, char* h1, int snps) {
    int j = 0, t = 0, t1 = 0, t2 = 0, choice = 0;
    char c;
    double parr0 = 0, parr1 = 0, sum = 0, rd = 0, b;
    if (snpfrag[snpfrag[start].component].best_mec <= 0) return -1;
    if (snpfrag[start].frags < 1) return -1;
        ...
        for (t1 = 0; t1 < Flist[j].blocks; t1++) {
            for (t2 = 0; t2 < Flist[j].list[t1].len; t2++) {
                ...
                if (t >= start && t <= end) {
                    ...
                    c = Flist[j].clust;
                    if (Flist[j].list[t1].hap[t2] != h1[t]) c = (char) (49 - (int) (c - 48));
                    if (c == '0') parr0 += b;
                    else parr0 -= b;
                }
            }
...
void frag_cluster_initialize(struct fragment* Flist, int fragments, struct SNPfrags* snpfrag, char* h1, int snps, struct BLOCK* clist, int comps) {
            ...
            for (i = 0; i < clist[k].frags - 1; i++) {
                ...
                for (j = i + 1; j < clist[k].frags; j++) {
                    ...
                    for (a = 0; a < Flist[t2].blocks; a++) {
                        for (b = 0; b < Flist[t2].list[a].len; b++) {
                            if (thap[Flist[t2].list[a].offset + b] == Flist[t2].list[a].hap[b]) thap[Flist[t2].list[a].offset + b] = 'M';
                            else thap[Flist[t2].list[a].offset + b] = 'm';
                        }
                    }
                    for (a = 0; a < Flist[t1].blocks; a++) {
                        for (b = 0; b < Flist[t1].list[a].len; b++) {
                            if (thap[Flist[t1].list[a].offset + b] == 'M') match++;
                            else if (thap[Flist[t1].list[a].offset + b] == 'm') mismatch++;
                        }
                    }
    ...
    for (i = 0; i < fragments; i++) {
        if (Flist[i].clust == 'x') Flist[i].clust = (char) (48 + (int) (drand48()*2 - 0.00001));
    }
    //for (i=0;i<snps;i++) { if (snpfrag[i].frags < 1 || h1[i] != '-') continue; h1[i] = '0';   sample_block(Flist,fragments,snpfrag,i,i,h1,snps); }
    for (i = 0; i < snps; i++) {
        if (snpfrag[i].frags < 1) continue;
        h1[i] = '0';
        sample_block(Flist, fragments, snpfrag, i, i, h1, snps);
    }
