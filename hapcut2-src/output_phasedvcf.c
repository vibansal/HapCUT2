...
/* one line of VCF file: https://github.com/samtools/htslib/blob/develop/htslib/vcf.h
https://github.com/samtools/htslib/blob/1.2.1/test/test-vcf-api.c
typedef struct {
    int32_t rid;  // CHROM
    int32_t pos;  // POS
...
int output_vcf_stilltobeused(char* vcffile, struct SNPfrags* snpfrag, int snps,char* H1,struct fragment* Flist, int fragments,char* outvcf) {
    char buffer[500000];
    char temp[1000];
    char GQ[5];
    char* gen;
