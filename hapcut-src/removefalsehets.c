...
float calculate_reduction_mecscore(struct fragment* Flist, int f, char* h, int variant) {
    int j = 0, k = 0;
    float good = 0, bad = 0;
    float prob = 0, prob1 = 0, origscore = 0, newscore = 0;
    float good1 = 0, bad1 = 0;
    for (j = 0; j < Flist[f].blocks; j++) {
        for (k = 0; k < Flist[f].list[j].len; k++) {
            ...
            if (Flist[f].list[j].offset + k != variant) {
                if (h[Flist[f].list[j].offset + k] == Flist[f].list[j].hap[k]) good1 += prob1;
                else bad1 += prob1;
            }
        }
    }
    if (good < bad) origscore = good;
    else origscore = bad;
    if (good1 < bad1) newscore = good1;
    else newscore = bad1;
...
float evaluate_variant(struct fragment* Flist, int fragments, struct SNPfrags* snpfrag, int v, char* HAP1) {
    ...
    for (i = 0; i < snpfrag[v].frags; i++) {
        ...
        {
            t = Flist[j].list[0].offset;
            if (HAP1[t] == '-') continue;
            if ((int) Flist[j].list[0].qv[0] - QVoffset < MINQ) continue;
            prob = QVoffset - (int) Flist[j].list[0].qv[0];
            prob /= 10;
            prob1 = log10(1.0 - pow(10, prob));
            if (Flist[j].list[0].hap[0] == '0') {
                snpfrag[v].A0++;
                snpfrag[v].G00 += prob1;
                snpfrag[v].G11 += prob;
                snpfrag[v].G01 += log10(0.5);
        ...
        for (t1 = 0; t1 < Flist[j].blocks; t1++) {
            for (t2 = 0; t2 < Flist[j].list[t1].len; t2++) {
                ...
                prob = QVoffset - (int) Flist[j].list[t1].qv[t2];
                prob /= 10;
                prob1 = log10(1.0 - pow(10, prob));
        ...
        if (Fnew1 > Fnew2) L00 += Fnew1 + log10(1.0 + pow(10, Fnew2 - Fnew1));
        else L00 += Fnew2 + log10(1.0 + pow(10, Fnew1 - Fnew2));
        if (Fnew3 > Fnew4) L11 += Fnew3 + log10(1.0 + pow(10, Fnew4 - Fnew3));
        else L11 += Fnew4 + log10(1.0 + pow(10, Fnew3 - Fnew4));
        if (Fnew5 > Fnew6) Lnovar += Fnew5 + log10(1.0 + pow(10, Fnew6 - Fnew5));
