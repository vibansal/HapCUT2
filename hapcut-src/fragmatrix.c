...
{
	int i=0,j=0,k=0,maxdeg=0,avgdeg=0, t1=0,t2=0; int csnps=0;
	...
	for (i=0;i<fragments;i++)
	{
		...
		for (j=0;j<vars-1;j++)
		{
			for (k=j+1;k<=j+2 && k <vars;k++) // two edges are added for each node except last two nodes, FOSMIDS =1 
			{	
				t1 = varlist[j]; t2 = varlist[k]; 
...
int determine_connected_components(struct fragment* Flist,int fragments,struct SNPfrags* snpfrag,int snps)
{
	...
	for (i=0;i<snps;i++) 
	{
		...
		snpfrag[i].rank = 0;  snpfrag[i].csize = 1;
	}
	...
	for (i=0;i<fragments;i++)
	{
		for (j=0;j<Flist[i].blocks;j++)
		{
			...
			{
				if (j ==0 &&  k==0) continue; 
				x = Flist[i].list[0].offset; y = Flist[i].list[j].offset + k; // the pair of SNPs
				parent_x = find_parent(snpfrag,x); parent_y = find_parent(snpfrag,y);
				if (parent_x == parent_y)  continue;
				else if (snpfrag[parent_x].rank < snpfrag[parent_y].rank) 
				{
					snpfrag[parent_x].component = parent_y; snpfrag[parent_y].csize += snpfrag[parent_x].csize; snpfrag[parent_x].csize = 0;
				}
				else if (snpfrag[parent_x].rank > snpfrag[parent_y].rank) 
				{
					snpfrag[parent_y].component = parent_x; snpfrag[parent_x].csize += snpfrag[parent_y].csize; snpfrag[parent_y].csize = 0;
				}
				else
				{
					snpfrag[parent_y].component = parent_x; snpfrag[parent_x].rank++; 
					snpfrag[parent_x].csize += snpfrag[parent_y].csize; snpfrag[parent_y].csize = 0;
				}
			}
		}
	}
...
void generate_clist_structure(struct fragment* Flist,int fragments,struct SNPfrags* snpfrag,int snps,int components,struct BLOCK* clist)
{
        ...
        for (i=0;i<snps;i++)
	{
		...
		clist[component].phased++; clist[component].lastvar = i; 
	}
        ...
        for (i=0;i<fragments;i++) 
	{
        ...
        for (i=0;i<components;i++) clist[i].flist = calloc(sizeof(int),clist[i].frags);     for (i=0;i<components;i++) clist[i].frags=0;
        for (i=0;i<fragments;i++)
        {
...
void update_snpfrags(struct fragment* Flist,int fragments,struct SNPfrags* snpfrag,int snps,int* components)
{
	...
	for (i=0;i<snps;i++) {snpfrag[i].frags = 0; snpfrag[i].ff = -1;}
        for (i=0;i<fragments;i++)
        {
        ...
        for (i=0;i<snps;i++) 
	{ 
		snpfrag[i].flist = (int*)malloc(sizeof(int)*snpfrag[i].frags); snpfrag[i].alist = (char*)malloc(snpfrag[i].frags);
	}

	for (i=0;i<snps;i++) 
	{ 
		snpfrag[i].component = -1; snpfrag[i].csize=1; snpfrag[i].frags=0; snpfrag[i].edges=0; 
	...
	for (i=0;i<fragments;i++)
	{
		...
		else if (FOSMIDS >=1)
		{
			...
			j = 0; snpfrag[Flist[i].list[j].offset].edges -= 1;  
			j = Flist[i].blocks-1; 	snpfrag[Flist[i].list[j].offset+Flist[i].list[j].len-1].edges -= 1;  
		}
	}
}
