...
struct fragment
{
				char* id; short blocks; struct block* list; char clust; // l indicates if it belongs to the haplotype or it's complement
				int component; float currscore;
				int calls; float ll;
...
struct BLOCK
{
				int offset; int length; int phased; char* haplotype; int* flist; int frags;
...
typedef struct EDGE { int s,t; float w; } EDGE;


struct SNPfrags 
{
				int* flist; int frags; char* alist; // alist is the alleles corresponding to each fragment that covers this SNP
				int component; int edges; // those that span the interval between this snp and the next snp 
				int csize; int blockno; 
				short best_mec;
				struct edge* elist;  int ff; 
				...
				int parent; float score;
				int revmap; int heaploc;
				char* id; char* allele0; char* allele1; char* chromosome; int position; 
				...
				int A0,A1; float G00,G01,G11; // allele counts and genotype likelihoods using singleton reads that cover only one variant
