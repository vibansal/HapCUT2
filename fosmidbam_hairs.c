...
struct BLOCK_READ
{
	...
	int firstread; int lastread; short reads; int start, end; 
	...
	double bscore1; int previous1; double score_window1; 
...
int cluster_reads(struct alignedread** readlist, int s,int e,FRAGMENT* flist,VARIANT* varlist,REFLIST* reflist)
{
	...
	for (i=0;i<blocks;i++) 
	{
		RL[i].reads=0; RL[i].firstread = -1; 
		for (j=0;j<10;j++) RL[i].subcounts[j] =0;
	}
	int creads = 0; int empty_blocks=0, block=0,nonempty_blocks=0;
	...
	for (i=s;i<e;i++)
	{
		...
		RL[block].reads++; RL[block].lastread = i; RL[block].end = readlist[i]->position; 
	...
	for (i=0;i<blocks;i+=BF) 
	{
		...
		empty = 0; for (j=0;j<BF;j++) empty += RL[i+j].reads; 
		if (empty ==0) empty_blocks++; else nonempty_blocks++;
		for (j=0;j<BF;j++) 
		{ 
	...
	for (i=0;i<blocks;i++) 
	{
		RL[i].previous = -1; RL[i].bscore = -10000000; RL[i].GC =0;
		RL[i].previous1 = -1; RL[i].bscore1 = -10000000; 
	...
	for (i=1;i<blocks;i++)
	{
		...
		RL[i].bscore = RL[i-1].bscore + score0;  RL[i].previous = i-1; 
		RL[i].bscore1 = RL[i-1].bscore1 + score0;  RL[i].previous1 = i-1; 
		RL[i].previous_cluster = RL[i-1].previous_cluster; RL[i].score_window = score0;

		j = i-1; reads = RL[i].reads; eb = 0; ws =0; ws_corrected=0; variance = RL[i].reads*RL[i].reads; 
		while ((i-j) < (int)(200000/BLOCK_SIZE) && j >= 0 && eb*BLOCK_SIZE < 10000) 
		{
			if (RL[j].reads > 0) eb = 0; else eb++; 
			reads += RL[j].reads; variance += RL[j].reads*RL[j].reads; 
			...
			if (reads >= 5 && ws_corrected >= 1000) 
			{
				...
				lambda = (double)reads/ws_corrected; blockscore = reads*(log(lambda)-1);
				...
				{
					RL[i].bscore = score0; RL[i].previous = (j-1); RL[i].reads_window = reads; 
					RL[i].score_window = blockscore;
					RL[i].previous_cluster = i;
					RL[i].variance = variance/(i-j) - mean*mean; RL[i].mean = mean;
				}
				...
				{
					RL[i].bscore1 = score0+prior_size; RL[i].previous1 = (j-1); RL[i].score_window1 = blockscore;
				}
			}
			j--;
		}
	...
	i = blocks-1; while (i >= 0)
	{
		...
		if (j < i-1) 
		{
			...
			first = RL[j+1].firstread; k=1; while(first < 0) { first = RL[j+1+k].firstread; k++; } 
			last = RL[i].lastread; k= 1;  while(last < 0) { last = RL[i-k].lastread; k++; } 
