...
int insert_keyvalue(HASHTABLE* ht, char* key,int slen,int value)
{
	...
	tempkey->value = value; tempkey->key = (char*)malloc(slen+1);
        int i=0; for (i=0;i<slen;i++) 	 tempkey->key[i] = key[i]; tempkey->key[i] = '\0'; 
	tempkey->next = ht->blist[hash]; ht->blist[hash] = tempkey; 
