...
int estimate_readdistance_distribution(struct alignedread** readlist,int s,int e)
{
	...
	int ndistances=0;
	int MAX_SIZE = 10000; 
	int intra_read_dist[MAX_SIZE]; for (i=0;i<MAX_SIZE;i++) intra_read_dist[i] = 0;
	float intra_read_pdf[MAX_SIZE]; for (i=0;i<MAX_SIZE;i++) intra_read_pdf[i] = 0.0;

	for (i=s;i<e-1;i++)
	{
		if (readlist[i]->IS < 0 ||  ((readlist[i]->flag & 1024) ==1024)) continue; 
		j = i+1; while (readlist[j]->IS < 0 ||  ((( (readlist[j]->flag & 1024) ==1024)) && j < e)) j++; 
		...
		{
			intra_read_dist[readlist[j]->position-readlist[i]->position] += 1.0; ndistances++;
		}
	}
...
int generate_single_fragment(struct alignedread** readlist, int s,int e,int length,double read_density,FRAGMENT* flist,VARIANT* varlist)
{
	...
	FRAGMENT fp; fp.variants = 0; fp.alist = (allele*)malloc(sizeof(allele)*unique_variants); 
...
{
	int i=0,prevpos = -1;
	FRAGMENT fragment; fragment.variants =0; fragment.alist = (allele*)malloc(sizeof(allele)*1024);
...
int print_clusters(struct alignedread** readlist, int s,int e,FRAGMENT* flist,VARIANT* varlist)
{
	...
	for (i=s;i<e-1;i++)
	{
		...
		else if (j < e && readlist[i]->cluster != readlist[j]->cluster) 
		{
			...
			if (reads >= 5) 
			{
				mean /= obs; std /= obs; std -= mean*mean;  if (std > 0) std = sqrt(std);
			...
			if (readlist[j]->mquality > 0) reads = 1; else reads = 0; 
			cluster_start = j; cluster_end = j; 
			prevpos = readlist[i]->position; 
			mean=0; std = 0; obs=0;
		}
	}
...
int init_clusters(struct alignedread** readlist,int s,int e)
{
        ...
        for (i=s;i<e;i++)
        {
                ...
                {
                        cluster_start = i; cl++;
                        prevtid = readlist[i]->tid; prevpos = readlist[i]->position;
                }
