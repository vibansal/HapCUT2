...
int parse_variant(VARIANT* variant, char* buffer,int samplecol)
{
	...
	variant->depth =0; variant->A1 =0; variant->A2 = 0; variant->Ae = 0;
	variant->H1 = 0; variant->H2 =0;
	...
	while (buffer[i] != '\n')
	{
		...
		if (col == samplecol)
		{
			variant->genotype = (char*)malloc(e-s+1); for (j=s;j<e;j++) variant->genotype[j-s] = buffer[j]; variant->genotype[j-s] = '\0';
		}
		else col++;
	}
	...
	{
		variant->allele1 = (char*)malloc(strlen(variant->RA)+1); strcpy(variant->allele1,variant->RA);
		j=0; while (variant->AA[j] != ',' && j < strlen(variant->AA)) j++; 
		variant->allele2 = (char*)malloc(j+1); for (i=0;i<j;i++) variant->allele2[i] = variant->AA[i]; variant->allele2[i] = '\0';
	...
	else if (variant->genotype[0] == '0' || variant->genotype[2] =='0') // at least one allele is reference
	{
		variant->allele1 = (char*)malloc(strlen(variant->RA)+1); strcpy(variant->allele1,variant->RA);
		j=0; while (variant->AA[j] != ',' && j < strlen(variant->AA)) j++;
		k=j+1; while (variant->AA[k] != ',' && k < strlen(variant->AA)) k++; 
		variant->allele2 = (char*)malloc(k-j+1); for (i=j+1;i<k;i++) variant->allele2[i-j-1] = variant->AA[i]; variant->allele2[i-j-1] = '\0';
		variant->type = strlen(variant->allele2) -strlen(variant->allele1);
	}
	...
	i = strlen(variant->allele1)-1; j = strlen(variant->allele2)-1;flag=0;
	while (i > 0 && j >0) 
	{
		if (variant->allele1[i] != variant->allele2[j]) break;
		i--; j--; flag++;
	}
	variant->allele1[i+1] = '\0'; variant->allele2[j+1] = '\0';
...
int read_variantfile(char* vcffile,VARIANT* varlist,HASHTABLE* ht,int* hetvariants,int samplecol)  
{
	...
	*hetvariants =0; int het=0;
	...
	while (fgets(buffer,100000,fp))
	{
		...
		{
			het = parse_variant(&varlist[i],buffer,samplecol);  (*hetvariants) += het;
...
int calculate_rightshift(VARIANT* varlist,int ss,REFLIST* reflist)
{
        ...
        else if (a1 ==1 && a2 > a1)
        {
                ...
                while (j-1 < reflist->lengths[reflist->current] && varlist[ss].allele2[i] == reflist->sequences[reflist->current][j-1] && i < a2) { i++; j++; shift++; } 
                if (i == a2) // covered the full length of the inserted bases
                {
                        i = varlist[ss].position; 
                        while (i-1 < reflist->lengths[reflist->current] && j-1 < reflist->lengths[reflist->current] && reflist->sequences[reflist->current][i-1] == reflist->sequences[reflist->current][j-1]) { i++; j++; shift++; } 
                }
                return shift; 
        }
        else return 0;
...
int read_variantfile_oldformat(char* snpfile,VARIANT* varlist,HASHTABLE* ht,int snps)
{
	...
	char allele1[256]; char allele2[256]; char genotype[256]; int quality; 
	char type[64]; char prevchrom[256]; char chrom[256]; 
	strcpy(prevchrom,"----");
 	int i=0;
	int chromosomes = 0;
	char buffer[10000];
	...
	for (i=0;i<snps;i++) 
	{
		...
		varlist[i].allele1 = (char*)malloc(strlen(allele1)+1); strcpy(varlist[i].allele1,allele1); 
		varlist[i].allele2 = (char*)malloc(strlen(allele2)+1); strcpy(varlist[i].allele2,allele2); 
		varlist[i].chrom = (char*)malloc(strlen(chrom)+1); strcpy(varlist[i].chrom,chrom);
		...
		else if (strcmp(type,"DNM") ==0) { varlist[i].type = 0;  }
		else if (strcmp(type,"DEL") ==0) { varlist[i].type = -1*strlen(allele1); } 
		else if (strcmp(type,"INS") ==0) { varlist[i].type = strlen(allele2); } 
		if (strcmp(varlist[i].chrom,prevchrom) !=0)
		{
