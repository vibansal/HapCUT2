...
// sort such that mate pairs are together and reverse sorted by starting position of second read in a mate-pair
int compare_fragments(const void *a,const void *b)
{
        FRAGMENT* f1 = (FRAGMENT*)a; FRAGMENT* f2 = (FRAGMENT*)b;
...
int print_matepair(FRAGMENT* f1, FRAGMENT* f2,VARIANT* varlist,FILE* outfile)
{
	...
	fprintf(outfile," "); for (i=0;i<f1->variants;i++) fprintf(outfile,"%c",f1->alist[i].qv);
	for (i=0;i<f2->variants;i++) fprintf(outfile,"%c",f2->alist[i].qv);
	fprintf(outfile,"\n");
...
{			
	...
	{
		// bug fixed here: mate-pairs being examined twice April 5 2012 
		i =first; 
		while (i< (*fragments)-1)
		{
