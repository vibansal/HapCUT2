...
{
	int offset = varlist[ss].position-start-l2; int j=0; char match = '0'; 
	for (j=0;j<strlen(varlist[ss].allele1);j++)
	{
		if (read->sequence[l1+offset+j] != varlist[ss].allele1[j]) { match = '-'; break; } 
	}
	if (match == '-')
	{
		match = '1';
		...
		{
			if (read->sequence[l1+offset+j] != varlist[ss].allele2[j]) { match = '-'; break; } 
		}
	}
	if (read->quality[l1+offset]-QVoffset >= MINQ && varlist[ss].type ==0)
	{
		varlist[ss].depth++; 
		if (match == '0') 
		{
		...
		{
			if ((read->flag & 16) == 16) varlist[ss].Ae += 1<<16; else varlist[ss].Ae += 1;
		}
	}

	if (match != '-' && read->quality[l1+offset]-QVoffset >= MINQ && varlist[ss].type ==0) 
	{
		fragment->alist[fragment->variants].varid = ss; fragment->alist[fragment->variants].allele = match;
...
int compare_read_INDEL(struct alignedread* read,VARIANT* varlist,int ss,int start,int l1,int l2,int clength,FRAGMENT* fragment,int CL,REFLIST* reflist)
{
	...
	int offset = varlist[ss].position-start-l2; int j=0,k=0; int a1=0,a2=0; 
	int s1=0,s2=0;
	...
	if (pflag)
	{
		if (offset-1 >= 0) { for (j=offset-1;j<clength;j++) fprintf(stdout,"%c",read->sequence[l1+j]); } 
		else { fprintf(stdout,"_"); for (j=offset;j<clength;j++) fprintf(stdout,"%c",read->sequence[l1+j]); }
	//fprintf(stdout," read\n");
		fprintf(stdout,"%s",varlist[ss].allele1); 
		for (j=offset-1+a1;j<k;j++) fprintf(stdout,"%c",reflist->sequences[reflist->current][start+l2-1+j]+32); fprintf(stdout," allele1 %d/%d\t",score1,b1);
		fprintf(stdout,"%s",varlist[ss].allele2); 
		for (j=offset-1+a2;j<k;j++) fprintf(stdout,"%c",reflist->sequences[reflist->current][start+l2-1+j+a1-a2]+32); fprintf(stdout," allele2 %d/%d\t",score2,b2);
	}
	...
	s1 = start-1+l2+clength-1; s2 = l1+clength-1;
	s1 = varlist[ss].position-2+a1-1; s2 -= start-1+l2+clength-1; s2 += varlist[ss].position-2+a1-1; 
	...
	if (s2 < clength)
	{
		...
		for (j=a2-1;j>=0;j--) 
		{ 
			if (s2 <0) { partialflag =1; break; }  // should be flagged so that if the read doesn't cover the full insertion to the left, it is not marked as allele 1
	...
	int allele=-1; int quality = 20; // base quality
	int op; 
...
int extract_variants_read(struct alignedread* read,HASHTABLE* ht,CHROMVARS* chromvars,VARIANT* varlist,int paired,FRAGMENT* fragment,int chrom,REFLIST* reflist)
{
	...
	{
		while(varlist[ss].position < start && ss < VARIANTS && ss <= chromvars[chrom].last) ss++; firstvar = ss;
	...
	for (i=0;i<read->cigs;i++)
	{
		...
		// consider =/X extended cigar operations as M | this should work fine  
		if (op == BAM_CMATCH || op == BAM_CEQUAL || op == BAM_CDIFF)
		{
			while (varlist[ss].position >= start+l2 && varlist[ss].position < start+l2 + ol && ss <= chromvars[chrom].last)
			{
				// function call 
				//printf("read->readid %s %d ss %d %c %d\n",read->readid,ov,ss,varlist[ss].heterozygous,varlist[ss].type);
				if (varlist[ss].heterozygous == '1' && varlist[ss].type ==0) 
				{
		...
		{  
			if (varlist[ss].position == start+l2 && varlist[ss].type == ol && varlist[ss].heterozygous == '1' && PARSEINDELS ==1 && ss <= chromvars[chrom].last) 
			{
				...
				fragment->alist[fragment->variants].varid = ss; fragment->alist[fragment->variants].allele = '1';
...
{
	int i=0;
        fnew->variants = fragment->variants; fnew->paired = 1;
...
{
	int i=0,sl=0;
        flist[fragments].variants = fragment->variants; flist[fragments].paired = fragment->paired;
	...
	if (BED_FILE ==1) 
	{
		if (fragment->paired ==1 && read->position < read->mateposition) sprintf(flist[fragments].id,"%s_%s:%d:%d",read->readid,read->chrom,read->position,read->mateposition-read->position); 
		else if (fragment->paired ==1) sprintf(flist[fragments].id,"%s_%s:%d:%d",read->readid,read->chrom,read->mateposition,read->position-read->mateposition); 
		else sprintf(flist[fragments].id,"%s_%s:%d:0",read->readid,read->chrom,read->position);
	}
        else
	{
		for (i=0;i<sl;i++) flist[fragments].id[i] = read->readid[i]; flist[fragments].id[i] = '\0';
	}
        return 1;
}
