...
void print_options()
{
	...
	fprintf(stderr,"--bed fragments : list of fragments (chr:start-end) for extracting synthetic long reads\n\n");
...
int parse_bamfile_sorted(char* bamfile,HASHTABLE* ht,CHROMVARS* chromvars,VARIANT* varlist,REFLIST* reflist)
{
	...
	char* temp_array = (char*)malloc(1024); // so that we can assign new id 
	...
	while (samread(fp, b) >= 0)
	{
		...
		if ((read->flag & 8) || absIS > MAX_IS || absIS < MIN_IS || read->IS ==0 || !(read->flag & 1) || read->tid != read->mtid) // single read
		{
			...
			if (chrom >=0 && PEONLY ==0) 
			{
				if (BED_FILE ==1)
				{
				//	sprintf(temp_array,"%s_%d_%d_SINGLE",read->readid,read->position,read->IS);
				//	fragment.id = temp_array;
				}
				//else 
				fragment.id = read->readid; fragment.paired = 0; 
				...
				{
					if (BED_FILE ==1) { add_fragment(flist,&fragment,read,fragments); fragments++; } 
					else print_fragment(&fragment,varlist,fragment_file);
					// edit 07/24/2015, add fragment to list to make things uniform rather than print it out straightaway
				}
			}
		}
		else  // paired-end read 
		{
			...
			fragment.variants =0; v1 =0; v2=0; fragment.paired = 1; 
...
int main (int argc, char** argv)
{
	...
	for (i=1;i<argc;i+=2)
	{
		...
		{
			fprintf(stderr,"\n The --variants option is deprecated and may not work correctly, use --VCF to provide variants in VCF format \n");  
		...
		else if (strcmp(argv[i],"--bed")==0)  BED_FILE = 1;  
		else if (strcmp(argv[i],"--barcode")==0)  { BARCODE = atoi(argv[i+1]); fprintf(stderr,"reads have barcode tag \n"); } 
		else if (strcmp(argv[i],"--counts")==0)  PRINT_COUNTS = 1;  
	...
	if (PRINT_COUNTS ==1)
	{
		...
		for (i=0;i<variants;i++)
		{
			...
			fprintf(stdout,"variant %d %s %d %d %s %s %d:%d %d:%d errors %d:%d\n",i+1,varlist[i].genotype,varlist[i].position-1,varlist[i].type,varlist[i].RA,varlist[i].AA,varlist[i].A1>>16,varlist[i].A1 & xor,varlist[i].A2>>16,varlist[i].A2 & xor,varlist[i].Ae>>16,varlist[i].Ae & xor);
		}
	}
	return 0;
}
