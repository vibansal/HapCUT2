...
#define USE_FREAD 1 // use fread vs fgetc function to read fasta files
...
int read_fastaheader(char* fastafile,REFLIST* reflist) // assumed to be reference.fasta.fai
{
	...
	char fastaindexfile[1024]; sprintf(fastaindexfile,"%s.fai",fastafile);
	FILE* fp; 
	...
	reflist->ns = 0; 
	while ( fgets(buffer,MAX_BUF_SIZE,fp) != NULL) reflist->ns++;
	fclose(fp);
	...
	while ( fgets(buffer,MAX_BUF_SIZE,fp) != NULL) 
	{
		...
		if (c1 < 0) c1 = c;
		for(k=j;k<c1 && k-j < 4096;k++) reflist->names[i][k-j] = buffer[k];
		...
		for(k=j;k<c;k++) 
		{
			reflist->lengths[i] *= 10; reflist->lengths[i] += (int)buffer[k]-48; 
		}
		...
		for(k=j;k<c;k++) 
		{
			reflist->offsets[i] *= 10; reflist->offsets[i] += (uint64_t)buffer[k]-48; 
		}
...
{
	if (chrom >= reflist->ns) { return -1; } 
	if (fseek(fp,reflist->offsets[chrom],SEEK_SET) != 0) { return -1; } 
	...
	int j=0,i=0,k=0,bases=0,n=0; 
	clock_t t = clock();
	static unsigned char buf[BUF_SIZE]; char c; 

	if (USE_FREAD ==1) 
	{
		while ((n = fread(buf, 1, BUF_SIZE, fp)) > 0)
		{
			for (i=0;i<n && bases < reflist->lengths[chrom];i++)
			{
				if (buf[i] != '\n' && buf[i] != '\t' && buf[i] != ' ') 
				{ 
					reflist->sequences[chrom][j++] = buf[i]; bases++; 
				} 
			}
			if (bases >= reflist->lengths[chrom]) break; 
		}
	}
	else
	{
...
//fread(). It reads much faster especially if buffer for fread is set to 65536
// new function that reads the chromosome 'chrom' directly from fasta file using seek
int read_chromosome(REFLIST* reflist,int chrom,FILE* fp)
{
	...
	if (USE_FREAD ==1) 
	{
		while ((n = fread(buf, 1, BUF_SIZE, fp)) > 0)
		{
			//fprintf(stderr,"seq: %d %d chrom %d\n",n,bases,reflist->lengths[chrom]);
			for (i=0;i<n && bases < reflist->lengths[chrom];i++)
			{
				if (buf[i] != '\n' && buf[i] != '\t' && buf[i] != ' ') 
				{ 
					reflist->sequences[chrom][j++] = toupper(buf[i]); bases++; 
					//fprintf(stdout,"%c\n",toupper(buf[i])); 
				} 
			}
			if (bases >= reflist->lengths[chrom]) break; 
	...
	else
	{
		...
		while (bases <reflist->lengths[chrom])
		{
			if (c != '\n' && c != '\t' && c != ' ')
			{
				reflist->sequences[chrom][j] = toupper(c); j++;
				//fprintf(stdout,"%c\n",toupper(c)); 
	...
	// now mark the targeted bases in lower case, not anymore
...
int read_next_chromosome(REFLIST* reflist,int chrom,FILE* fp)
{
	...
	if (reflist->intervals <=0) { fprintf(stderr,"\n");  return 1;  }
	i = reflist->first_interval_chrom[chrom]; 
...
// updated 08/12/16 to use samtools kseq.h 
...
{
	char buffer[MAX_BUF_SIZE]; int s=0,c=0,i=0,e=0,j=0,isann=0;
	char chrom[1024]; int start; int end; char prevchrom[1024]; char annotation[1024];
	strcpy(prevchrom,"-");
	int index =0;
	reflist->intervals = 0; reflist->intervallist = NULL;  
	if (strcmp(bedfile,"None")==0) return -1;  // no bedfile
	FILE* fp = fopen(bedfile,"r");
	if (fp == NULL) { fprintf(stderr,"bed file %s not found, please check the filename\n",bedfile); return -1; } 
	...
	while (fgets(buffer,MAX_BUF_SIZE,fp) != NULL) 
	{
		...
		while(buffer[c] == ' ' || buffer[c] == '\t') c++; s = c; while (buffer[c] != ' ' && buffer[c] != '\t') c++; e = c;
		start =0; for (i=s;i<e;i++) start = start*10 + (int)buffer[i]-48;

		while(buffer[c] == ' ' || buffer[c] == '\t') c++; s = c;
		while (buffer[c] != ' ' && buffer[c] != '\t' && buffer[c] != '\n' && isdigit(buffer[c])) c++; e = c;
		end =0; for (i=s;i<e;i++) end = end*10 + (int)buffer[i]-48;

		// annotation can be used to store barcodes for each interval 	
		isann = 0;	
		if (buffer[c] != '\n')
		{
			while(buffer[c] == ' ' || buffer[c] == '\t') c++; s = c;
		...
		if (strcmp(chrom,prevchrom) !=0)
		{
			...
			for (i=0;i<reflist->ns;i++)
			{
				if (strcmp(chrom,reflist->names[i]) == 0) { index = i; break; }
			}
		...
		if (index != -1)
		{
			reflist->intervallist[j].chrom = index; reflist->intervallist[j].start = start; reflist->intervallist[j].end = end;
			if (isann ==1) 
			{
